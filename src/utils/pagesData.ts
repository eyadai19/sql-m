...
AND DATE_HIRED < CURRENT_DATE - INTERVAL '90 days';`,
  ...
  dataTypes: {
    exerciseParams: {
        ...
        prompt: "Analyze the employees table and identify the correct data types for a new employee record. Write a query to insert a new employee named 'Mark Wilson' as a 'Software Engineer' with a salary of 65000, hired on '2024-03-15', with status 'active' in department 1.",
        tables: ["employees"],
        difficulty: "Easy",
        hints: [
...
VALUES ('Mark Wilson', 'Software Engineer', 1, 65000, '2024-03-15', 'active');`,
    ...
    explanationParams: {
        ...
        howItWorks: "SQL data types define the type of value that can be stored in a table column. Each column in a table must have a specified data type that determines what kind of data can be stored in it and how that data can be used.",
...
CREATE TABLE employees (
    ...
    last_working_day DATE NULL
            ...
            explanation: "This example shows the data types used in our employees table. INTEGER is used for IDs, VARCHAR for text fields with specific lengths, DECIMAL for precise numeric values like salary, and DATE for temporal data.",
        },
        notes: [
        ...
        difficulty: "Beginner",
        tags: ["Data Types", "SQL Fundamentals", "Database Design", "Column Types"],
    },
},
references: {
  exerciseParams: {
      title: "Primary and Foreign Key Implementation",
      prompt: "Write a query that demonstrates the relationship between departments and employees using primary and foreign keys. List all departments and show how many employees each department has, including departments with no employees.",
      tables: ["employees", "departments"],
      difficulty: "Medium",
      hints: [
...
ORDER BY d.id;`,
  ...
  explanationParams: {
      ...
      howItWorks: "Primary keys (PK) and foreign keys (FK) are fundamental database concepts that ensure data integrity and establish relationships between tables. A primary key uniquely identifies each record in a table, while a foreign key references a primary key in another table, creating a relationship between them.",
...
WHERE d.id = 1;`,
          explanation: "This query demonstrates how the primary key (id) in the departments table is referenced by the foreign key (department_id) in the employees table, creating a relationship between departments and their employees.",
      },
      notes: [
          "Primary Keys (PK):",
...
ERD: {
  exerciseParams: {
      ...
      prompt: "Looking at the relationship between the employees and departments tables, identify the type of relationship and list all the attributes that establish this relationship. Then write a query that demonstrates this relationship by showing each department and its employees.",
      tables: ["employees", "departments"],
      difficulty: "Medium",
      hints: [
...
ORDER BY d.name, e.name;`,
  ...
  explanationParams: {
      ...
      howItWorks: "An ERD visualizes database structure by showing entities (tables) and their relationships. It uses specific symbols to represent different types of relationships between tables, such as one-to-one, one-to-many, or many-to-many.",
...
N:M (Many-to-Many)   →  ──>│<──`,
      example: {
          code: `/* Current Database ERD:
			...
			last_working_day
			*/`,
          explanation: "This ERD shows that one department can have multiple employees (one-to-many relationship). The department_id in the Employees table is a foreign key that references the id in the Departments table. PK indicates Primary Key, and FK indicates Foreign Key.",
      },
      notes: [
      ...
      difficulty: "Beginner",
      tags: ["ERD", "Database Design", "Relationships", "Data Modeling", "Primary Key", "Foreign Key"],
  }},
  createTable: {
    exerciseParams: {
      title: "Creating Employee Table",
      prompt: "Write a CREATE TABLE statement for the employees table with appropriate data types and constraints. Include columns for id (primary key), name, position, department_id (foreign key), salary, date_hired, and status.",
      tables: [],
      difficulty: "Medium",
      hints: [
  ...
  FOREIGN KEY (department_id) REFERENCES departments(id)
    ...
    explanationParams: {
      ...
      howItWorks: "The CREATE TABLE statement is used to create a new table in a database. It defines the table's structure, including column names, data types, and constraints. This statement is fundamental to database schema design.",
      syntax: `CREATE TABLE table_name (
  ...
  location VARCHAR(100)
        ...
        explanation: "This example creates a departments table with five columns. The id is set as the primary key, and most fields are marked as NOT NULL to ensure data integrity. The budget field uses DECIMAL for precise monetary values."
      },
      notes: [
      ...
      difficulty: "Beginner",
      tags: ["DDL", "CREATE TABLE", "Schema Design", "Constraints", "Data Types"]
    }
  },
  schema: {
    exerciseParams: {
      title: "Database Schema Design",
      prompt: "Create a schema definition for a database that includes both the employees and departments tables, ensuring proper relationships and constraints between them.",
      tables: ["employees", "departments"],
      difficulty: "Hard",
      hints: [
...
CREATE INDEX idx_emp_status ON company.employees(status);`,
    ...
    explanationParams: {
      ...
      howItWorks: "A database schema is a blueprint that defines the structure of a database. It includes tables, relationships, constraints, and other database objects. Schema definition involves creating a logical organization of data that ensures data integrity and efficient querying.",
...
CREATE TABLE hr.departments (
  ...
  name VARCHAR(100) NOT NULL
        ...
        explanation: "This example creates an HR schema and a departments table within it. The schema provides a namespace for organizing database objects and controlling access permissions."
      },
      notes: [
      ...
      difficulty: "Advanced",
      tags: ["DDL", "Schema Design", "Database Architecture", "Constraints", "Indexing"]
    }
  },
  alterTable: {
    exerciseParams: {
      title: "Modifying Table Structure",
      prompt: "Write ALTER TABLE statements to add a 'performance_rating' column to the employees table and modify the salary column to have a minimum value constraint.",
      tables: ["employees"],
      difficulty: "Medium",
      hints: [
...
ADD CONSTRAINT salary_min_check CHECK (salary >= 30000);`,
    ...
    explanationParams: {
      ...
      howItWorks: "The ALTER TABLE statement modifies an existing table's structure. It can add or remove columns, modify data types, add or remove constraints, and rename table elements. This statement is crucial for maintaining and evolving database schemas.",
...
ADD CONSTRAINT budget_check CHECK (budget > 0);`,
        explanation: "This example adds a description column to the departments table and adds a check constraint to ensure the budget is always positive."
      },
      notes: [
      ...
      difficulty: "Intermediate",
      tags: ["DDL", "ALTER TABLE", "Schema Modification", "Constraints", "Database Management"]
    }
  },
  dropTable: {
    exerciseParams: {
      title: "Removing Database Objects",
      prompt: "Write DROP statements to safely remove the employees and departments tables, considering their relationship and the proper order of removal.",
      tables: ["employees", "departments"],
      difficulty: "Easy",
      hints: [
...
DROP TABLE IF EXISTS departments;`,
    ...
    explanationParams: {
      ...
      howItWorks: "The DROP statement removes database objects such as tables, schemas, or entire databases. It's a powerful DDL command that permanently deletes both the structure and data of the specified objects.",
      syntax: `DROP TABLE [IF EXISTS] table_name [CASCADE | RESTRICT];
DROP SCHEMA [IF EXISTS] schema_name [CASCADE | RESTRICT];`,
      example: {
        code: `DROP TABLE IF EXISTS employees CASCADE;
DROP SCHEMA IF EXISTS company CASCADE;`,
        explanation: "This example safely removes the employees table and the company schema, using CASCADE to automatically remove dependent objects. The IF EXISTS clause prevents errors if the objects don't exist."
      },
      notes: [
      ...
      difficulty: "Beginner",
      tags: ["DDL", "DROP TABLE", "Schema Management", "Database Administration", "Data Definition"]
    }
  },
  where: {
	exerciseParams: {
		title: "Basic Employee Filter",
		prompt: "Write a query to find all active employees in the Engineering department (id=1) who earn more than $50,000.",
		tables: ["employees"],
		difficulty: "Easy",
		hints: [
	...
	WHERE status = 'active'
	  ...
	  explanationParams: {
		...
		howItWorks: "The WHERE clause filters rows based on specified conditions. It's used to retrieve only the data that meets certain criteria, making queries more specific and useful.",
	...
	WHERE department_id = 2
	  AND salary >= 50000
	  AND status = 'active';`,
		  explanation: "This query finds active employees in department 2 who earn $50,000 or more, demonstrating multiple conditions in the WHERE clause.",
		},
		notes: [
  ...
  subQuery: {
	exerciseParams: {
		...
		prompt: "Write a query to find all employees who earn more than the average salary in their department.",
		tables: ["employees"],
		difficulty: "Hard",
		hints: [
	...
	WHERE salary > (
	  ...
	  explanationParams: {
		...
		howItWorks: "A subquery is a query nested inside another query. It can be used in various parts of the main query (SELECT, FROM, WHERE) to perform complex data retrieval and filtering operations.",
		syntax: `SELECT columns
	FROM table1
	WHERE column1 IN (
	  SELECT column2
	...
	ORDER BY salary DESC;`,
		  explanation: "This query finds employees who earn more than the company-wide average salary, demonstrating a simple subquery in the WHERE clause.",
		},
		notes: [
  ...
  operator: {
	exerciseParams: {
		...
		prompt: "Write a query to find all employees who either earn more than $80,000 OR work in the Engineering department (id=1) AND were hired before 2022.",
		tables: ["employees"],
		difficulty: "Medium",
		hints: [
	...
	WHERE (salary > 80000 OR department_id = 1)
	  ...
	  explanationParams: {
		...
		howItWorks: "Logical operators (AND, OR, NOT) combine multiple conditions in SQL queries. They follow Boolean logic rules and can be used to create complex filtering conditions.",
		syntax: `SELECT columns
	FROM table_name
	WHERE condition1 AND condition2
	   OR condition3
	...
	WHERE (salary >= 60000 AND department_id = 1)
	   OR (salary >= 75000 AND department_id = 2);`,
		  explanation: "This query demonstrates combining conditions with AND/OR to find employees meeting specific salary requirements in different departments.",
		},
		notes: [
		  "AND has higher precedence than OR",
  ...
  like: {
	exerciseParams: {
		...
		prompt: "Write a query to find all employees whose names start with 'J' or contain 'son'.",
		tables: ["employees"],
		difficulty: "Easy",
		hints: [
	...
	WHERE name ILIKE 'J%'
	  ...
	  explanationParams: {
		...
		howItWorks: "The LIKE operator performs pattern matching using wildcards. It's commonly used for searching text fields where exact matches aren't required or known.",
		syntax: `SELECT columns
	FROM table_name
	...
	WHERE 
	  ...
	  OR name LIKE 'M%';`,
		  explanation: "This query finds employees with 'Engineer' anywhere in their position title or names starting with 'M'.",
		},
		notes: [
  ...
  groupBy: {
	exerciseParams: {
		...
		prompt: "Write a query to show the number of employees and average salary for each department, but only for departments with more than 5 employees.",
		tables: ["employees", "departments"],
		difficulty: "Medium",
		hints: [
	...
	HAVING COUNT(*) > 5;`,
	  ...
	  explanationParams: {
		...
		howItWorks: "The GROUP BY clause groups rows that have the same values in specified columns into summary rows. It's often used with aggregate functions to perform calculations on each group.",
	...
	HAVING COUNT(*) >= 3;`,
		  explanation: "This query groups employees by department and shows various statistics, but only for departments with 3 or more employees.",
		},
		notes: [
  ...
  distinct: {
	exerciseParams: {
		...
		prompt: "Write a query to find all unique job positions in the company, ordered alphabetically.",
		tables: ["employees"],
		difficulty: "Easy",
		hints: [
	...
	ORDER BY position;`,
	  ...
	  explanationParams: {
		...
		howItWorks: "The DISTINCT clause eliminates duplicate rows from the result set. When used with multiple columns, it considers the combination of all specified columns for uniqueness.",
	...
	ORDER BY department_id, position;`,
		  explanation: "This query finds unique combinations of departments and positions, showing the distribution of job roles across departments.",
		},
		notes: [
  ...
  between: {
	exerciseParams: {
		...
		prompt: "Write a query to find all employees with salaries between $50,000 and $75,000.",
		tables: ["employees"],
		difficulty: "Easy",
		hints: [
	...
	ORDER BY salary;`,
	  ...
	  explanationParams: {
		...
		howItWorks: "The BETWEEN operator selects values within a given range, inclusive of the boundary values. It can be used with numbers, dates, and strings, providing a cleaner alternative to using >= and <= operators.",
		syntax: `SELECT column1, column2
	FROM table_name
	...
	WHERE date_hired BETWEEN '2023-01-01' AND '2023-12-31'
	  AND salary BETWEEN 45000 AND 85000;`,
		  explanation: "This query finds employees hired in 2023 with salaries between $45,000 and $85,000, demonstrating BETWEEN with both dates and numbers.",
		},
		notes: [
  ...
  aggregateFunction: {
	exerciseParams: {
		...
		prompt: "Write a query using aggregate functions to find the total salary cost, average salary, and count of employees per department.",
		tables: ["employees", "departments"],
		difficulty: "Medium",
		hints: [
		  "Use GROUP BY with department_id",
	...
	GROUP BY d.id, d.name;`,
	  ...
	  explanationParams: {
		...
		howItWorks: "Aggregate functions perform calculations on a set of rows and return a single result. Common functions include COUNT, SUM, AVG, MAX, and MIN. They're often used with GROUP BY to analyze data by categories.",
		syntax: `SELECT 
	  column1,
	  COUNT(*),
	  SUM(column2),
	...
	FROM employees
	GROUP BY department_id;`,
		  explanation: "This query calculates various statistics about employee salaries grouped by department, including the count of employees, total salary cost, average salary, and salary range.",
		},
		notes: [
		...
		difficulty: "Intermediate",
		tags: ["Aggregate Functions", "GROUP BY", "SQL Analytics", "Data Analysis"],
	  },
  },
  innerJoin: {
    exerciseParams: {
      title: "Basic Table Relationships",
      prompt: "Write a query to demonstrate a basic INNER JOIN between two related tables.",
      tables: ["employees", "departments"],
      difficulty: "Easy",
      hints: [
...
WHERE d.name = 'Engineering' AND e.status = 'active';`,
    ...
    explanationParams: {
      ...
      howItWorks: "INNER JOIN creates a result set containing only the records where the specified columns match in both tables. It's the most common type of join, used when you want to retrieve only complete, matching data sets.",
      syntax: `SELECT columns
FROM table1 t1
...
WHERE e.salary > 70000;`,
        explanation: "Demonstrates a basic INNER JOIN retrieving matching records based on a relationship between tables.",
      },
      notes: [
      ...
      difficulty: "Beginner",
      tags: ["INNER JOIN", "Basic Queries", "Data Relationships", "SQL Fundamentals"],
    },
  },
  leftJoin: {
    exerciseParams: {
      title: "Preserving Source Records",
      prompt: "Write a query that keeps all records from the left table while matching available data from the right table.",
      tables: ["employees", "departments"],
      difficulty: "Medium",
      hints: [
...
ORDER BY e.salary DESC;`,
    ...
    explanationParams: {
      ...
      howItWorks: "LEFT JOIN retains all records from the left (first) table and matches them with records from the right table where possible. When no match exists, NULL values are used for the right table's columns.",
      syntax: `SELECT columns
FROM table1 t1
...
WHERE e.status = 'active';`,
        explanation: "Shows how LEFT JOIN preserves all records from the first table while matching available data from the second table.",
      },
      notes: [
      ...
      difficulty: "Intermediate",
      tags: ["LEFT JOIN", "Data Preservation", "Optional Relationships", "SQL Joins"],
    },
  },
  rightJoin: {
    exerciseParams: {
      title: "Right Table Data Preservation",
      prompt: "Create a query that maintains all records from the right table while matching data from the left table where possible.",
      tables: ["employees", "departments"],
      difficulty: "Medium",
      hints: [
...
GROUP BY d.name, d.manager;`,
    ...
    explanationParams: {
      ...
      howItWorks: "RIGHT JOIN preserves all records from the right (second) table while matching records from the left table where possible. Unmatched records from the left table result in NULL values.",
      syntax: `SELECT columns
FROM table1 t1
...
RIGHT JOIN employees e ON e.department_id = d.id
GROUP BY d.name;`,
        explanation: "Demonstrates RIGHT JOIN usage by showing all departments and their employee counts, including departments with no employees.",
      },
      notes: [
  ...
  outerJoin: {
    exerciseParams: {
      ...
      prompt: "Write a query that combines all records from both tables, matching where possible and preserving unmatched records.",
      tables: ["employees", "departments"],
      difficulty: "Hard",
      hints: [
...
ORDER BY e.salary DESC NULLS LAST;`,
    ...
    explanationParams: {
      ...
      howItWorks: "FULL OUTER JOIN combines all records from both tables, matching records where possible and including unmatched records from both tables with NULL values for missing data.",
      syntax: `SELECT columns
FROM table1 t1
...
FROM employees e
FULL OUTER JOIN departments d ON e.department_id = d.id;`,
        explanation: "Shows complete data integration between employees and departments, including unmatched records from both tables.",
      },
      notes: [
      ...
      difficulty: "Advanced",
      tags: ["FULL OUTER JOIN", "Data Integration", "Complete Data Sets", "SQL Joins"],
    },
  },
  selfJoin: {
    exerciseParams: {
      title: "Self-Referential Relationships",
      prompt: "Create a query that joins a table with itself to explore hierarchical or self-referential relationships.",
      tables: ["employees"],
      difficulty: "Hard",
      hints: [
...
ORDER BY e2.name;`,
    ...
    explanationParams: {
      ...
      howItWorks: "SELF JOIN connects a table to itself, typically using different aliases, to analyze hierarchical or self-referential relationships within the same dataset.",
      syntax: `SELECT columns
FROM table1 t1
...
FROM employees e1
LEFT JOIN employees e2 ON e1.manager_id = e2.id;`,
        explanation: "Demonstrates how to use SELF JOIN to show employee-manager relationships within the same table.",
      },
      notes: [
      ...
      difficulty: "Advanced",
      tags: ["SELF JOIN", "Hierarchical Data", "Recursive Queries", "SQL Joins"],
    },
  },
  join: {
    exerciseParams: {
      title: "General Join Operations",
      prompt: "Write a query demonstrating the versatile use of JOIN with additional conditions and filtering.",
      tables: ["employees", "departments"],
      difficulty: "Medium",
      hints: [
...
ORDER BY e.salary DESC;`,
    ...
    explanationParams: {
      ...
      howItWorks: "JOIN (without a specific type) defaults to INNER JOIN, providing a flexible way to combine related tables based on matching values in specified columns.",
      syntax: `SELECT columns
FROM table1 t1
...
WHERE d.budget > 300000;`,
        explanation: "Shows basic JOIN usage with additional filtering conditions to combine related data from multiple tables.",
      },
      notes: [
      ...
      difficulty: "Intermediate",
      tags: ["JOIN", "Data Relationships", "SQL Fundamentals", "Query Building"],
    },
  },
};
