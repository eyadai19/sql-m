...
export default function PostCard({
	...
	const handleDelete = async () => {
		try {
			...
			if (onPostDelete) {
				onPostDelete(post.id); // استدعاء الدالة لتحديث حالة posts
			}
			return true;
		} catch (err) {
	...
	return (
		<CommentProvider initialComments={post.comments}>
				...
				{showComments && (
					<div className="space-y-6 border-t bg-muted/10 p-4 sm:p-6">
						<CommentForm
							...
							userPhoto={useImage}
						/>
						<CommentList postCommentLikeAction={postCommentLikeAction} />
					</div>
				)}
			</Card>
		</CommentProvider>
