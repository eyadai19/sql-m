...
import type { DatabaseRow, TablesSnapshot, OperationType, QueryResult } from '@/lib/types/mockDatabase';
...
export class TempDatabase {
    ...
    try {
      ...
      for (const emp of employees) {
        await client.query(
          ...
          [emp.name, emp.position, emp.department_id, emp.salary, emp.date_hired, emp.status, emp.last_working_day]
        );
      }
  ...
  async executeQuery(query: string): Promise<QueryResult> {
    ...
    try {
      ...
      if (operationType === 'SELECT') {
        ...
        const affectedRows = this.getAffectedRows(tablesBefore, tablesAfter, operationType);
  ...
  private getAffectedRows(before: TablesSnapshot, after: TablesSnapshot, operationType: OperationType) {
    const columns = ['id', 'name', 'position', 'department_id', 'salary', 'date_hired', 'status', 'last_working_day'];
    ...
    switch (operationType) {
        ...
        return {
          ...
          rows: after.employees.rows
            .filter((row: DatabaseRow) => 
              !before.employees.rows.find((r: DatabaseRow) => r.id === row.id))
      ...
      case 'UPDATE':
        return {
          ...
          rows: after.employees.rows
            .filter((row: DatabaseRow) => {
              const beforeRow = before.employees.rows.find((r: DatabaseRow) => r.id === row.id);
              return beforeRow && JSON.stringify(beforeRow) !== JSON.stringify(row);
            })
            .map((row: DatabaseRow) => Object.values(row))
        };
