...
// // 		);
// // 		const tablesWithColumnsAndData = await Promise.run(
// // 			tables.map(async (table: { name: string }) => {
// // 				const columns = await db.run(`PRAGMA table_info(${table.name});`);
// // 				const data = await db.run(`SELECT * FROM ${table.name}`);
// // 				return {
...
export async function GET(req: NextRequest) {
	try {
		...
		const relations = await Promise.all(
			tables.map(async (table: { name: string }) => {
				...
				return {
					...
					relations: foreignKeys.map((key: any) => ({
						table: key.table,
						from: key.from,
						to: key.to,
					})),
		...
		return NextResponse.json(
			{
				error: "حدث خطأ أثناء جلب العلاقات بين الجداول.",
				details: error instanceof Error ? error.message : String(error),
			},
			{ status: 500 },
		);
